...
{
    public class M05UltimateAttack03AttackingController : BaseM05UltimateAttack03
    {
        private const float FreezeChargeEffectPositionY = 0.6f;
        private const float ChargeEffectDuration = 1.7f;
        private const float EffectLargeScale = 1.5f;
        private const float MoveTowardAmount = 0.1f;
        private const string AnimatorTrigger = "attack_special_3";

        public override CharacterAnimator CharacterAnimator { get; set; }
        public MapDirection Direction { get; set; }
        public AudioClip LastKickClip { get; set; }
        ...
        internal override Sequence Initialize()
        {
            ...
            DOTween.Sequence()
                .AppendInterval(0.6f)
                .Append(root.DOLocalMove(_originalPosition + new Vector3(moveX, 0, moveZ), 0.0f));
        ...
        {
            ShowChargeEffect(FreezeChargeEffectPositionY);
        }
        ...
        private void Attack(bool isPlaySound = false)
        {
            if (isPlaySound)
            {
                _audioSource.clip = LastKickClip;
                _audioSource.Play();
            }
        ...
        private void ShowChargeEffect(float y = -1)
        {
            var originPos = Effect.transform.localPosition;
            // if (y != -1)
            // {
            //     Effect.transform.localPosition = new Vector3(Effect.transform.localPosition.x, y, Effect.transform.localPosition.z);
            // }
            
            Effect.transform.localPosition = new Vector3(-0.01f, 0.177f, 0f);
